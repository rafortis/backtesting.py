...
class Trade:
    ...
    def __repr__(self):
        return f'<Trade size={self.__size} time={self.__entry_bar}-{self.__exit_bar or ""} ' \
               f'price={self.__entry_price}-{self.__exit_price or ""} pl={self.pl:.0f} entry={self.__entry_rule} exit={self.__exit_type}>'
    ...
    def pl_pct(self):
        ...
        price = self.__exit_price or self.__broker.last_price
        return (copysign(1, self.__size) * (price / self.__entry_price - 1)) - ((self.__broker._fixedCommission/(copysign(1, self.__size)*self.__size))/self.__entry_price)
